...
def create_sibling_reconciler(
    ...
    2. Both present + inconsistent: ✗ Raise ValidationError
    3. Only ID: Extrapolate label from enum
    4. Only label (known): Extrapolate ID from enum
    5. Only label (unknown): Set ID=99 (OTHER)
    6. Neither present: ✓ Accept (both None)
    ...
    def reconcile(cls: type[Any], data: Any) -> Any:
        ...
        if id_value is not None and label_value is not None:
            ...
            try:
                ...
                # Invalid enum value - let Pydantic handle it
                pass
            return data
