...
class BaseEvent(OcsfObject):
    ...
    __profiles__: dict[str, OcsfObject]
    ...
    @classmethod
    def _get_profile_class(cls, profile: str):
        if cls.allowed_profiles is None or profile not in cls.allowed_profiles:
            raise ValueError(profile)
        try:
            m = importlib.import_module(f"ocsf.profiles.{profile}")
        except ModuleNotFoundError:
            raise ValueError(profile)
        for _, pcls in inspect.getmembers(m, inspect.isclass):
            if issubclass(pcls, OcsfObject) and getattr(pcls, "schema_name", None) == profile:
                return pcls
        raise ValueError(profile)

    @model_validator(mode="wrap")
    @classmethod
    def handle_type_fields(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_profiles(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        obj: Self = handler(data)
        if obj.metadata and obj.metadata.profiles:
            for profile_name in obj.metadata.profiles:
                if profile_name not in cls.allowed_profiles:
                    raise ValueError(f"`{profile_name}` not allowed for {cls.__name__}")
                profile_class = cls._get_profile_class(profile_name)
                obj.__profiles__[profile_name] = profile_class.model_validate(obj)
        return obj

    def __getitem__[T: OcsfObject](self, profile_class: type[T]) -> T:
