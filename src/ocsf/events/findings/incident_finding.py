...
from pydantic import AnyUrl, Field, ModelWrapValidatorHandler, computed_field, model_validator
...
class IncidentFinding(BaseEvent):
    ...
    @classmethod
    def validate_activity_name(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        ...
        if instance.__pydantic_extra__ and "activity_name" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("activity_name")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_confidence(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        ...
        instance = handler(data)
        if instance.__pydantic_extra__ and "confidence" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("confidence")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_impact(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "impact" in data and "impact_id" not in data:
            impact = re.sub(r"\W", "_", data.pop("impact").upper())
            data["impact_id"] = ImpactId[impact]
        instance = handler(data)
        if instance.__pydantic_extra__ and "impact" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("impact")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_priority(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        ...
        instance = handler(data)
        if instance.__pydantic_extra__ and "priority" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("priority")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_status(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "status" in data and "status_id" not in data:
            status = re.sub(r"\W", "_", data.pop("status").upper())
            data["status_id"] = StatusId[status]
        instance = handler(data)
        if instance.__pydantic_extra__ and "status" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("status")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_verdict(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        ...
        instance = handler(data)
        if instance.__pydantic_extra__ and "verdict" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("verdict")
        return instance
