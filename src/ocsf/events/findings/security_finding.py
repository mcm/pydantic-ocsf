...
class SecurityFinding(BaseEvent):
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_state(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "state" in data and "state_id" not in data:
            state = re.sub(r"\W", "_", data.pop("state").upper())
            data["state_id"] = StateId[state]
        instance = handler(data)
        if instance.__pydantic_extra__ and "state" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("state")
        return instance
