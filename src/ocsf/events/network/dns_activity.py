...
class DnsActivity(Network):
    ...
    @classmethod
    def validate_rcode(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "rcode" in data and "rcode_id" not in data:
            rcode = re.sub(r"\W", "_", data.pop("rcode").upper())
            data["rcode_id"] = RcodeId[rcode]
        instance = handler(data)
        if instance.__pydantic_extra__ and "rcode" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("rcode")
        return instance
