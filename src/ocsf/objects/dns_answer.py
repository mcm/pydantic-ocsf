...
class DnsAnswer(Dns):
    ...
    @classmethod
    def validate_flags(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "flags" in data and "flag_ids" not in data:
            flags = re.sub(r"\W", "_", data.pop("flags").upper())
            data["flag_ids"] = FlagIds[flags]
        instance = handler(data)
        if instance.__pydantic_extra__ and "flags" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("flags")
        return instance
