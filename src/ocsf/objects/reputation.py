...
class Reputation(Object):
    ...
    @classmethod
    def validate_score(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "score" in data and "score_id" not in data:
            score = re.sub(r"\W", "_", data.pop("score").upper())
            data["score_id"] = ScoreId[score]
        instance = handler(data)
        if instance.__pydantic_extra__ and "score" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("score")
        return instance
