...
from pydantic import IPvAnyAddress, ModelWrapValidatorHandler, computed_field, model_validator
...
class ResourceDetails(Resource):
    ...
    @classmethod
    def validate_role(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "role" in data and "role_id" not in data:
            role = re.sub(r"\W", "_", data.pop("role").upper())
            data["role_id"] = RoleId[role]
        instance = handler(data)
        if instance.__pydantic_extra__ and "role" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("role")
        return instance
