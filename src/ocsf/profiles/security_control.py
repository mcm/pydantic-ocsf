...
class SecurityControl(BaseProfile):
    ...
    @classmethod
    def validate_action(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        if "action" in data and "action_id" not in data:
            action = re.sub(r"\W", "_", data.pop("action").upper())
            data["action_id"] = ActionId[action]
        instance = handler(data)
        if instance.__pydantic_extra__ and "action" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("action")
        return instance
    ...
    @model_validator(mode="wrap")
    @classmethod
    def validate_disposition(cls, data: dict[str, Any], handler: ModelWrapValidatorHandler) -> Self:
        ...
        instance = handler(data)
        if instance.__pydantic_extra__ and "disposition" in instance.__pydantic_extra__:
            instance.__pydantic_extra__.pop("disposition")
        return instance
