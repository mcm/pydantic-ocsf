...
class QueryEvidenceTcpStateId(IntEnum):
    """The state of the TCP socket for the network connection.

    See: https://schema.ocsf.io/1.5.0/data_types/query_evidence_tcp_state_id
    """
    ...
    SYN_RECEIVED = 3  # The socket has passively received a connection request from a remote peer.
    FIN_WAIT_1 = 4  # The socket connection has been closed by the local application, the remote peer has not yet acknowledged the close, and the system is waiting for it to close its half of the connection.
    FIN_WAIT_2 = 5  # The socket connection has been closed by the local application, the remote peer has acknowledged the close, and the system is waiting for it to close its half of the connection.
