...
class NetworkEndpointNetworkScopeId(IntEnum):
    ...
    See: https://schema.ocsf.io/1.7.0/data_types/network_endpoint_network_scope_id
    """

    UNKNOWN = 0  # Unknown whether this endpoint resides within the customer’s network.
    INTERNAL = 1  # The endpoint resides inside the customer’s network.
    EXTERNAL = 2  # The endpoint is on the Internet or otherwise external to the customer’s network.
    OTHER = 99  # The network scope is not mapped. See the <code>network_scope</code> attribute, which contains a data source specific value.
